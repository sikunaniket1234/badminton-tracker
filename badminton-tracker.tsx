...
import { Trophy, IndianRupee, Plus, X, Users, History, Calendar, CheckCircle } from 'lucide-react';

export default function BadmintonTracker() {
  const [currentUser, setCurrentUser] = useState(null);
  const [loginUsername, setLoginUsername] = useState('');
  const [loginPassword, setLoginPassword] = useState('');
  const [fines, setFines] = useState({ aniketnayak: 0, souravssk: 0 });
  ...
  const loadData = async () => {
    try {
      const finesData = await window.storage.get('fines');
      const matchesData = await window.storage.get('matches');
      const settlementsData = await window.storage.get('settlements');
      
      if (finesData) {
        setFines(JSON.parse(finesData.value));
      }
      if (matchesData) {
        setMatches(JSON.parse(matchesData.value));
      }
      if (settlementsData) {
        setSettlements(JSON.parse(settlementsData.value));
      }
    } catch (error) {
  ...
  const saveData = async (newFines, newMatches, newSettlements) => {
    try {
      await window.storage.set('fines', JSON.stringify(newFines || fines));
      await window.storage.set('matches', JSON.stringify(newMatches || matches));
      await window.storage.set('settlements', JSON.stringify(newSettlements || settlements));
    } catch (error) {
  ...
  const addFine = async () => {
    ...
    newFines[currentUser] += parseInt(fineAmount);
  ...
  const handleSettlement = async () => {
    ...
    const newSettlements = [settlement, ...settlements];
  ...
  const getMonthlyReport = (yearMonth) => {
    ...
    monthMatches.forEach(match => {
      if (match.type === 'fine') {
        fineData[match.player] += match.amount;
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-400 to-blue-500 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full">
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg text-xs text-gray-600">
            ...
            <p>Aniket: aniket123</p>
            <p>Sourav: sourav123</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex justify-between items-center mb-2">
            <div>
              ...
              <p className="text-sm opacity-90">Hello, {users[currentUser].displayName}!</p>
            </div>
            <button
                  ...
                  {matches.slice(0, 5).map(match => (
                    <div key={match.id} className="border-l-4 border-blue-500 pl-4 py-2">
                      {match.type === 'match' ? (
                        ...
                        <div>
                          <p className="font-semibold">
                            💰 {users[match.player].displayName} - Fine
                          </p>
                  ...
                  <div key={settlement.id} className="border rounded-lg p-4 bg-green-50">
                    ...
                    <div className="space-y-1 text-sm">
                      <p className="text-gray-700">
                        <span className="font-medium">{users[settlement.payer].displayName}</span> paid{' '}
                        <span className="font-medium">{users[settlement.receiver].displayName}</span>
                      </p>
              ...
              <div className="space-y-4">
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-sm text-gray-600 mt-2">
                    {fineBalance > 0 
                      ? `${users[currentUser].displayName} pays ${users[otherUser].displayName}`
                      : `${users[otherUser].displayName} pays ${users[currentUser].displayName}`
                    }
                  </p>
                </div>
                        ...
                        {report.monthSettlements.map(settlement => (
                          <div key={settlement.id} className="bg-white rounded p-3 text-sm">
                            <p className="font-medium">
                              {users[settlement.payer].displayName} → {users[settlement.receiver].displayName}: ₹{settlement.balance}
                            </p>
